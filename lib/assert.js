 ...
 */
export function mustBeSimilarTo(one, two, msg) {
  if (!assert.similar(one, two)) {
    throw new AssertionError(`'${inspect(one)}' must be similar to '${inspect(two)}'.`, msg);
  }
}
 ...
 */
export function mustNotBeSimilarTo(one, two, msg) {
  if (!assert.notSimilar(one, two)) {
    throw new AssertionError(`'${inspect(one)}' must not be similar to '${inspect(two)}'.`, msg);
  }
}
